...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:gco="http://www.isotc211.org/2005/gco" xmlns:gmd="http://www.isotc211.org/2005/gmd" xmlns:gmi="http://www.isotc211.org/2005/gmi" xmlns:srv="http://www.isotc211.org/2005/srv" xmlns:gmx="http://www.isotc211.org/2005/gmx" xmlns:gsr="http://www.isotc211.org/2005/gsr" xmlns:gss="http://www.isotc211.org/2005/gss" xmlns:gts="http://www.isotc211.org/2005/gts" xmlns:gml="http://www.opengis.net/gml/3.2" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:nc="http://www.unidata.ucar.edu/namespaces/netcdf/ncml-2.2" exclude-result-prefixes="nc">
    <xd:doc xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" scope="stylesheet">
        ...
        Recent Modifications
            <xd:p>2022-08-05. Construct link to point to the udunits2.xml located in the same folder with the ncml. Zafer Defne zdefne@usgs.gov </xd:p>
            <xd:p>2019-03-27. Using Anna Milan's suggestion of OnRequest for units vocabulary. Updated to have Godiva link. Zafer Defne zdefne@usgs.gov </xd:p>
    ...
    <!--Modified on 2022-08-04 to construct link to the udunits2.xml in same folder with the ncml -->
    <xsl:template name="pathname">
        <xsl:param name="flocation2" />
        <xsl:choose>
            <xsl:when test="contains($flocation2, '/')">
                <xsl:call-template name="pathname">
                    <xsl:with-param name="flocation2" select="substring-after($flocation2, '/')" />
                </xsl:call-template>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="concat( substring-before($flocation, $flocation2), 'udunits2_combined_whmsc.xml#')" />
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    <xsl:variable name="flocation" as="xs:string*" select="concat('https:', substring-after(replace(nc:netcdf/@location, 'dodsC', 'fileServer'), ':'))"/>
    ...
    <xsl:variable name="verticalMin" select="(/nc:netcdf/nc:group[@name='CFMetadata']/nc:attribute[@name='geospatial_vertical_min']/@value,
    ...
    <xsl:variable name="verticalMax" select="(/nc:netcdf/nc:group[@name='CFMetadata']/nc:attribute[@name='geospatial_vertical_max']/@value,
    ...
    <xsl:template match="/">
        <gmi:MI_Metadata>
            <xsl:attribute name="xsi:schemaLocation">
                <xsl:value-of select="'http://www.isotc211.org/2005/gmi https://data.noaa.gov/resources/iso19139/schema.xsd'"/>
            </xsl:attribute>
            <gmd:fileIdentifier>
                    ...
                    <xsl:when test="count($longitudeVariableName) + count($latitudeVariableName) + count($verticalVariableName) + count($timeVariableName)">
                        <gmd:MD_GridSpatialRepresentation>
                            <gmd:numberOfDimensions>
                                <gco:Integer>
                                    <xsl:value-of select="number(count($longitudeVariableName) > 0) + number(count($latitudeVariableName) > 0) + number(count($verticalVariableName) > 0) + number(count($timeVariableName) > 0)"/>
                                </gco:Integer>
                            </gmd:numberOfDimensions>
                                        ...
                                        <gmd:MD_DigitalTransferOptions>
                                            <gmd:onLine>
                                                <gmd:CI_OnlineResource>
                                                    ...
                                                    <gmd:description>
                                                        <gco:CharacterString>This URL points to Godiva2, a browser based viewer.</gco:CharacterString>
                                                    </gmd:description>
                                                    <gmd:function>
                    ...
                    <xsl:when test="$variableUnits">
                        <gmd:units>
                            <xsl:attribute name="xlink:href">
                                <!--xsl:value-of select="concat( substring-before($flocation,$fname), 'udunits2_combined_whmsc.xml#')"/-->
                                <xsl:call-template name="pathname">
                                    <xsl:with-param name="flocation2" select="$flocation"/>
                                </xsl:call-template>
    ...
    <xsl:template name="writeService">
        ...
        <gmd:identificationInfo>
            <xsl:element name="srv:SV_ServiceIdentification">
                ...
                <srv:couplingType>
                    <srv:SV_CouplingType codeList="https://data.noaa.gov/resources/iso19139/schema/resources/Codelist/gmxCodelists.xml#SV_CouplingType" codeListValue="tight">tight</srv:SV_CouplingType>
                </srv:couplingType>
